...
angular.module('world.controllers', [])
  ...
  .controller('mapCtrl', function ($scope, $rootScope, Property) {
    ...
    $scope.showMeTheCondo = function (position) {
      ...
      Property.query({
        ...
        for (var i = 0; i < res.length; i++) {
          ...
          var contentString = '<h2> <a href="/worldresiden/info.php?id='+res[i].obj._id+'" target="_blank">' + res[i].title + '</a></h2> ' +
          '<p>ค่าเช่า' + res[i].obj.rent + ' บาท</p><br>';

          if (res[i].images && res[i].images.length > 0) {
            contentString += '<img src="' + res[i].obj.images[3] + '"><br>';
          };
